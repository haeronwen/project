...
def get_store_groceries(store):
    ...
    Matches items to the standard basket and applies monthly quantities.
    store: 'lidl' (physical) or 'rohlik' (online delivery)
    Note: raw data item matching is approximate - will be improved with better scraping.
    """
    ...
    # get cheapest price per item category
    cheapest = raw.groupby("item")["price"].min().reset_index()

    # join with basket quantities
    merged = basket.merge(cheapest, on="item", how="left")

    # use store price where available, fall back to basket default
    merged["final_price"] = merged["price"].fillna(merged["unit_price"])

    monthly_total = (merged["final_price"] * merged["qty_per_month"]).sum()
    return float(monthly_total)
...
with right:
    ...
    if housing_type == "Apartment":
        ...
        if len(row) == 0:
            ...
            st.stop()
        housing_cost = float(row["avg_rent_per_person"].values[0])
    else:
    ...
    # get grocery cost based on store selection
    if grocery_store == "Physical store":
        ...
        groceries = None  # compute_monthly_cost will use the default from loaders
    ...
    with table_col:
        ...
        if housing_type == "Apartment":
            data_source = "scraped from Bezrealitky.cz"
        else:
            data_source = "scraped from rehos.cuni.cz"
        st.caption(
...
if housing_type == "Apartment":
    ...
    # Chart 1: same room type across all zones
    st.markdown(f"**{disposition} — rent per person by zone**")
    same_disp_rows = []
    for label, row in apt_df.iterrows():
        if f"Flat {disposition} " in label:
            same_disp_rows.append((label, row))

    cols = st.columns(len(same_disp_rows)) if same_disp_rows else st.columns(1)
    zone_order = ["Center", "Inner city", "Outer/suburbs"]
    same_disp_rows.sort(key=lambda x: next(
        (i for i, z in enumerate(zone_order) if z in x[0]), 99))

    for col, (label, row) in zip(cols, same_disp_rows):
        zone_name = label.replace(f"Flat {disposition} (", "").rstrip(")")
        is_current = zone_display in label
        title = f"{zone_name}{' (yours)' if is_current else ''}"
        col.metric(title, fmt(row["housing"]))

    # Chart 2: smaller/larger options in the same zone
    ...
    if adjacent:
        ...
        for disp in adjacent:
            # try to find the disposition in the chosen zone
            match = None
            for label, row in apt_df.iterrows():
                if f"Flat {disp} " in label and f"({zone_display})" in label:
                    match = (disp, row["housing"])
                    break
            # fall back to any zone if not found
            if match is None:
                for label, row in apt_df.iterrows():
